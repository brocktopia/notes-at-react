  ...
  Note.remove({notebook: req.params.notebookId}).exec().
    then(function (notes) {
      ...
      if (notes.ok) {
        ...
        res.status(500).send(new Error('Failed to delete notes'));
      }
  });
};
// Get Notebook & Notes (append notes to notebook object)
exports.read_a_notebook = function(req, res) {
  console.log(`appController.read_a_notebook() id: ${req.params.notebookId}`);
  Notebook.findById(req.params.notebookId).exec()
    .then(function(notebook) {
      if (!notebook) {
        console.warn('appController.read_a_notebook() No Data');
    ...
    .catch((err) => {
      console.log(`appController.read_a_notebook() Error: ${err.message}`);
      res.status(500).send(err);
    }
  )
};
    ...
    .exec(
      function(err, notes) {
        ...
        if (notes)
          ...
          console.warn('appController.notes_by_notebook() No Data');
          res.status(404).send('Not Found');
        }
...
exports.read_a_note = function(req, res) {
  console.log('appController.read_a_note() find note by id: ' + req.params.noteId);
